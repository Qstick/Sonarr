    ...
    public class add_custom_formats : NzbDroneMigrationBase
    {
        ...
        private void MigratePreferredTerms(IDbConnection conn, IDbTransaction tran)
        {
            ...
            using (var cmd = conn.CreateCommand())
            {
                ...
                cmd.CommandText = "SELECT Preferred, Name, IncludePreferredWhenRenaming, Enabled FROM ReleaseProfiles WHERE Preferred IS NOT NULL";
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            name = $"Unnamed_{unamedCount++}";
                        }

                        if (updatedCollections.Any(x => x.Name.ToLowerInvariant() == name.ToLowerInvariant()))
                        {
                            name = $"{name}_{unamedCount++}";
                        }
